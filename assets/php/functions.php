...
// Define the getFollowSuggestions() function
function filterFollowSuggestion()
{
    $list = getFollowSuggestions();
    $filter_list = array();
...
function getFollowSuggestions()
{
    ...
    // Query the database to retrieve users based on "following of following" list
    $query = "SELECT users.* FROM users
              INNER JOIN follow_list AS fl1 ON users.id = fl1.user_id
              INNER JOIN follow_list AS fl2 ON fl1.follower_id = fl2.user_id
              WHERE users.id != $current_user AND fl2.follower_id = $current_user
              GROUP BY users.id
              ORDER BY COUNT(DISTINCT fl1.follower_id) DESC
              LIMIT 10";
